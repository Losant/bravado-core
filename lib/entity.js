...
const buildEntityActions = function(actions) {
  const entityActions = {};
  for (const a in actions) {
    ...
    entityActions[a].href = actionTemplate.expand(this.body);
...
Entity.prototype.toJSON = function(options) {
  ...
  const obj = assign({}, this.body);
  obj._type = this.type;
  if (options.actions && this.actions) {
    obj._actions = buildEntityActions(this.actions);
  }
