...
const JWT_OPTIONS = {
  algorithm: process.env.JWT_ALGO || 'ES256',
  issuer: process.env.JWT_ISSUER
};
if (process.env.JWT_AUDIENCE) { JWT_OPTIONS.audience = process.env.JWT_AUDIENCE; }
const JWT_SECRET = process.env.JWT_SECRET;

const JWT_VALID_ALGOS = process.env.JWT_VALID_ALGOS || [ JWT_OPTIONS.algorithm ];
const JWT_VALID_ISSUERS = process.env.JWT_VALID_ISSUERS || [ JWT_OPTIONS.issuer ];
...
export default function(event, context) {
  ...
  context.verifyToken = function(token) {
    return jwt.verify(token, JWT_SECRET, { ...JWT_OPTIONS, algo: JWT_VALID_ALGOS, issuer: JWT_VALID_ISSUERS });
  };
