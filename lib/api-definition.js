...
ApiDefinition.build = function(root) {
  const index = path.resolve(root, 'api.yaml');
  const resourceDir = path.resolve(root, 'resources');
  const defDir = path.resolve(root, 'definitions');
  const exampleDir = path.resolve(root, 'examples');
  return q.nfcall(fs.readFile, index)
    ...
    .spread(function readDefs(doc, files) {
      return q.all(files.map(function(file) {
        ...
        if (ext === '.json' || ext === '.yaml' || ext === '.yml') {
          const base = path.basename(file, ext);
          const rel = path.relative(defDir, file);
